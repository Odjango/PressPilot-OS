 ...
 *
 * @package presspilot-simple-business-v1-2
 */

if (!function_exists('presspilot_simple_business_v1_2_setup')) {
    /**
     * Theme setup.
     */
    function presspilot_simple_business_v1_2_setup()
    {
        ...
        register_nav_menus(
            array(
                'primary' => __('Primary Menu', 'presspilot-simple-business-v1-2'),
            )
        );
    }
}
add_action('after_setup_theme', 'presspilot_simple_business_v1_2_setup');

/**
 * Enqueue front-end styles if needed.
 * Keep empty or minimal to encourage theme.json-driven styling.
 */
function presspilot_simple_business_v1_2_enqueue_styles()
{
    // Intentionally minimal: rely on theme.json for most styling.
    wp_enqueue_style(
        'presspilot-simple-business-v1-2-style',
        get_stylesheet_uri(),
        array(),
        wp_get_theme()->get('Version')
    );
}
add_action('wp_enqueue_scripts', 'presspilot_simple_business_v1_2_enqueue_styles');

/**
 * Activation Logic: Create Pages & Navigation
 */
add_action( 'after_switch_theme', 'presspilot_simple_business_v1_2_activate' );
function presspilot_simple_business_v1_2_activate() {
    $destructive = true;
    ...
    // Create Pages
    $pages_config = [
        'home' => ['title' => 'Home', 'content' => 'Content for Home'],
        'services' => ['title' => 'Services', 'content' => 'Content for Services'],
        'contact' => ['title' => 'Contact', 'content' => 'Content for Contact'],
    ];
    ...
    update_option('blogname', 'Brightline Consulting');
}
