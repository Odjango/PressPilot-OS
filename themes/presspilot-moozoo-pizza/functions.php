 ...
 */
function presspilot_moozoo_pizza_seed_content() {
	// Debug logging
	error_log( 'MooZoo Pizza Seeding: Checking...' );
	...
	$version     = '1.1.1.1764635014';
	$stored_ver  = get_option( 'presspilot_moozoo_pizza_seed_version' );

	// Already seeded for this version.
	if ( $stored_ver === $version ) {
		error_log( 'MooZoo Pizza Seeding: Already run for ' . $version );
		return;
	}

	error_log( 'MooZoo Pizza Seeding: Running for ' . $version );
	...
	$pages = array(
		'Home'    => 'Welcome to MooZoo Pizza, your cozy neighborhood slice shop.',
		'Menu'    => 'Explore our hand-crafted pizzas, salads, and desserts.',
		'About'   => 'Learn how MooZoo Pizza grew from a tiny oven to a local favorite.',
		'Services'=> 'Catering, delivery, and office lunches made easy.',
		'Blog'    => 'Behind-the-scenes stories from the MooZoo Pizza kitchen.',
...
HTML;
    ...
    // For Home page, use "MooZoo Pizza" instead of "Home"
    $hero_title_text = ($title === 'Home') ? 'MooZoo Pizza' : $title;
...
HTML;
		...
		if ( ! is_wp_error( $page_id ) ) {
			...
			error_log( 'MooZoo Pizza Seeding: Page already exists: ' . $title );
		} else {
			error_log( 'MooZoo Pizza Seeding: Failed to create page ' . $title . ': ' . $page_id->get_error_message() );
		}
	}
      ...
      foreach ( $nav_blueprint as $item ) {
		...
		$nav_post_id = wp_insert_post(
			array(
				...
				'post_name'    => 'presspilot-moozoo-pizza-primary-navigation',
				'post_author'  => $user_id,
			)
		);

		if ( ! is_wp_error( $nav_post_id ) ) {
			error_log( 'MooZoo Pizza Seeding: Created Navigation Post' );
...
HTML;
      ...
      $header_post_id = wp_insert_post( array(
          'post_title'   => 'header',
          'post_name'    => 'presspilot-moozoo-pizza//header',
      ...
      if ( ! is_wp_error( $header_post_id ) ) {
           ...
           error_log( 'MooZoo Pizza Seeding: Wired Header Template Part' );
      } else {
           error_log( 'MooZoo Pizza Seeding: Failed to wire Header: ' . $header_post_id->get_error_message() );
      }

		} else {
			error_log( 'MooZoo Pizza Seeding: Failed to create Navigation Post' );
		}
	}

	update_option( 'presspilot_moozoo_pizza_seed_version', $version );
}
add_action( 'admin_init', 'presspilot_moozoo_pizza_seed_content' );
 ...
 */
function presspilot_moozoo_pizza_nuke_templates() {
    // Use a generated timestamp so EVERY new build triggers a reset, not just version bumps.
    $build_id = '1764635015025'; 
    $nuke_key = 'presspilot_moozoo_pizza_nuke_build_id';
    
    if ( get_option( $nuke_key ) === $build_id ) {
        return;
    }

    error_log( 'MooZoo Pizza: New Build Detected (' . $build_id . '). Nuking stale database templates...' );
    ...
    if ( $query->have_posts() ) {
        while ( $query->have_posts() ) {
            ...
            error_log( 'MooZoo Pizza: Deleted stale template: ' . get_the_title() );
        }
    }
    wp_reset_postdata();

    // FORCE RE-SEEDING: Delete the seed version option so content generation runs again.
    delete_option( 'presspilot_moozoo_pizza_seed_version' );
    error_log( 'MooZoo Pizza: Reset seed version to force content generation.' );

    update_option( $nuke_key, $build_id );
    error_log( 'MooZoo Pizza: Nuke complete for build ' . $build_id );
}
add_action( 'init', 'presspilot_moozoo_pizza_nuke_templates' );
