 ...
 *
 * @package presspilot-repro-heavy-v1
 */

if (!function_exists('presspilot_repro_heavy_v1_setup')) {
    /**
     * Theme setup.
     */
    function presspilot_repro_heavy_v1_setup()
    {
...
add_action('after_setup_theme', 'presspilot_repro_heavy_v1_setup');

/**
 * Enqueue front-end styles if needed.
 * Keep empty or minimal to encourage theme.json-driven styling.
 */
function presspilot_repro_heavy_v1_enqueue_styles()
{
    // Intentionally minimal: rely on theme.json for most styling.
    wp_enqueue_style(
        'presspilot-repro-heavy-v1-style',
        get_stylesheet_uri(),
        array(),
        wp_get_theme()->get('Version')
    );
}
add_action('wp_enqueue_scripts', 'presspilot_repro_heavy_v1_enqueue_styles');

/**
 * AI Theme Seeder: Automates clean install and content injection.
 */
class presspilot_repro_heavy_v1_Seeder {
    ...
    public function activation_logic() {
        ...
        if ( get_option( 'presspilot_repro_heavy_v1_seeded' ) ) {
            return;
        }
        ...
        // 6. Set Flag
        update_option( 'presspilot_repro_heavy_v1_seeded', true );
    }
    ...
    private function create_pages() {
        ...
        $pages_config = array(
            'undefined' => array(
                'title' => 'undefined',
                'content' => '<!-- wp:paragraph {} --><p>Content for undefined</p><!-- /wp:paragraph -->'
            ),
            'undefined' => array(
                'title' => 'undefined',
                'content' => '<!-- wp:paragraph {} --><p>Content for undefined</p><!-- /wp:paragraph -->'
    ...
    private function update_site_options( $pages ) {
        ...
        update_option( 'blogname', 'PressPilot Repro Heavy' );
...
new presspilot_repro_heavy_v1_Seeder();
