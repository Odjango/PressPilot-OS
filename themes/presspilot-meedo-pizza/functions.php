 ...
 * Theme supports.
 */
if ( ! function_exists( 'presspilot_meedo_pizza_setup' ) ) :
	function presspilot_meedo_pizza_setup() {
...
add_action( 'after_setup_theme', 'presspilot_meedo_pizza_setup' );

/**
 * Pattern category for any Onion-specific patterns.
 */
function presspilot_meedo_pizza_register_pattern_category() {
	register_block_pattern_category(
		'presspilot-meedo-pizza',
		array(
			'label' => __( 'PressPilot / Meedo Pizza', 'presspilot-meedo-pizza' ),
		)
	);
}
add_action( 'init', 'presspilot_meedo_pizza_register_pattern_category' );

/**
 * One-time content seed.
 */
function presspilot_meedo_pizza_seed_content() {
	// Debug logging
	error_log( 'Meedo Pizza Seeding: Checking...' );
	...
	$version     = '1.1.1.1764597699';
	$stored_ver  = get_option( 'presspilot_meedo_pizza_seed_version' );

	// Already seeded for this version.
	if ( $stored_ver === $version ) {
		error_log( 'Meedo Pizza Seeding: Already run for ' . $version );
		return;
	}

	error_log( 'Meedo Pizza Seeding: Running for ' . $version );
	...
	update_option( 'blogname', 'Meedo Pizza' );
	...
	$pages = array(
		'Home'    => 'Welcome to Meedo Pizza, your cozy neighborhood slice shop.',
		'Menu'    => 'Explore our hand-crafted pizzas, salads, and desserts.',
		'About'   => 'Learn how Meedo Pizza grew from a tiny oven to a local favorite.',
		'Services'=> 'Catering, delivery, and office lunches made easy.',
		'Blog'    => 'Behind-the-scenes stories from the Meedo Pizza kitchen.',
...
HTML;
    ...
    // For Home page, use "Meedo Pizza" instead of "Home"
    $hero_title_text = ($title === 'Home') ? 'Meedo Pizza' : $title;
...
HTML;
		...
		if ( ! is_wp_error( $page_id ) ) {
			...
			error_log( 'Meedo Pizza Seeding: Page already exists: ' . $title );
		} else {
			error_log( 'Meedo Pizza Seeding: Failed to create page ' . $title . ': ' . $page_id->get_error_message() );
		}
	}
      ...
      foreach ( $nav_blueprint as $item ) {
		...
		$nav_post_id = wp_insert_post(
			array(
				...
				'post_name'    => 'presspilot-meedo-pizza-primary-navigation',
				'post_author'  => $user_id,
			)
		);

		if ( ! is_wp_error( $nav_post_id ) ) {
			error_log( 'Meedo Pizza Seeding: Created Navigation Post' );
...
HTML;
      ...
      $header_post_id = wp_insert_post( array(
          'post_title'   => 'header',
          'post_name'    => 'presspilot-meedo-pizza//header',
      ...
      if ( ! is_wp_error( $header_post_id ) ) {
           ...
           error_log( 'Meedo Pizza Seeding: Wired Header Template Part' );
      } else {
           error_log( 'Meedo Pizza Seeding: Failed to wire Header: ' . $header_post_id->get_error_message() );
      }

		} else {
			error_log( 'Meedo Pizza Seeding: Failed to create Navigation Post' );
		}
	}

	update_option( 'presspilot_meedo_pizza_seed_version', $version );
}
add_action( 'admin_init', 'presspilot_meedo_pizza_seed_content' );
 ...
 */
function presspilot_meedo_pizza_nuke_templates() {
    // Use a generated timestamp so EVERY new build triggers a reset, not just version bumps.
    $build_id = '1764597699083'; 
    $nuke_key = 'presspilot_meedo_pizza_nuke_build_id';
    
    if ( get_option( $nuke_key ) === $build_id ) {
        return;
    }

    error_log( 'Meedo Pizza: New Build Detected (' . $build_id . '). Nuking stale database templates...' );
    ...
    if ( $query->have_posts() ) {
        while ( $query->have_posts() ) {
            ...
            error_log( 'Meedo Pizza: Deleted stale template: ' . get_the_title() );
        }
    }
    wp_reset_postdata();

    // FORCE RE-SEEDING: Delete the seed version option so content generation runs again.
    delete_option( 'presspilot_meedo_pizza_seed_version' );
    error_log( 'Meedo Pizza: Reset seed version to force content generation.' );

    update_option( $nuke_key, $build_id );
    error_log( 'Meedo Pizza: Nuke complete for build ' . $build_id );
}
add_action( 'init', 'presspilot_meedo_pizza_nuke_templates' );
