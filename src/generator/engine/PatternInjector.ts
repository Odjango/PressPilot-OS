    ...
    async injectHeavyMode(themeDir: string, personality: ThemePersonality, userData: GeneratorData, safeName: string, contentJson: any): Promise<void> {
        ...
        if (personality.home_template) {
            ...
            let homeMarkup = getUniversalHomeContent(
                ...
                userData.name || 'PressPilot Site',
                (contentJson.pages || userData.pages || []) as { title: string, slug: string }[],
                Boolean(userData.logo)
            ).trim();

            // Replace token placeholders generated by section patterns.
            homeMarkup = this.applySlotReplacements(homeMarkup, contentJson.slots);
            ...
            const includeHeaderPart = userData.heroLayout !== 'fullBleed';
