# PressPilot Generator Constitution (V1.2)
...
5. **The Type Safety Rule**: All data flows through typed interfaces. New fields require updates to `PageContent` in `types.ts`.

## Capabilities (Phase 2.0)
...
- **Rich Content**: Chef/team data, about paragraphs injected via ContentEngine page-specific logic.
...
## Data Flow Contract
```
GeneratorData → ContentBuilder → ContentJSON → ContentEngine → Templates
```

All field names, placeholders, and mappings documented in `docs/GENERATOR_API.md`.

## Failure Philosophy
If a rule is broken, the process halts. We do not "fix" broken themes; we fix the Generator.

## Related Documents
- `docs/generator-constitution.md` — Detailed technical rules
- `docs/GENERATOR_API.md` — Field reference and content flow
