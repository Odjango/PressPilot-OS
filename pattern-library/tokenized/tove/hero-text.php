<?php
/**
 * Title: Hero with headings, a paragraph of text, and buttons.
 * Slug: tove/hero-text
 * Categories: tove-hero
 */
?>
<!-- wp:group {"align":"wide","layout":{"inherit":false,"contentSize":"710px","type":"constrained"}} -->
<div class="wp-block-group alignwide">
	<!-- wp:heading {"textAlign":"center","level":6,"backgroundColor":"primary","textColor":"foreground"} -->
	<h6 class="has-text-align-center has-foreground-color has-primary-background-color has-text-color has-background wp-block-heading">
		Now serving...</h6>
	<!-- /wp:heading -->

	<!-- wp:heading {"textAlign":"center","level":1} -->
	<h1 class="has-text-align-center wp-block-heading">{{HERO_TITLE}}</h1>
	<!-- /wp:heading -->

	<!-- wp:paragraph {"align":"center","fontSize":"large"} -->
	<p class="has-text-align-center has-large-font-size">{{HERO_TEXT}}</p>
	<!-- /wp:paragraph -->

	<!-- wp:buttons {"layout":{"type":"flex","justifyContent":"center","orientation":"horizontal"}} -->
	<div class="wp-block-buttons"><!-- wp:button -->
		<div class="wp-block-button"><a class="wp-block-button__link wp-element-button">{{HERO_CTA}}</a></div>
		<!-- /wp:button -->

		<!-- wp:button -->
		<div class="wp-block-button"><a class="wp-block-button__link wp-element-button">{{HERO_CTA_SECONDARY}}</a></div>
		<!-- /wp:button -->
	</div>
	<!-- /wp:buttons -->
</div>
<!-- /wp:group -->