<?php
/**
 * Title: Follow us section with a vertical layout.
 * Slug: tove/general-follow-us-vertical
 * Categories: tove-general
 */
?>
<!-- wp:group {"backgroundColor":"senary","className":"is-style-tove-shaded"} -->
<div class="wp-block-group is-style-tove-shaded has-senary-background-color has-background">
	<!-- wp:heading {"textAlign":"center"} -->
	<h2 class="has-text-align-center">Follow Us!</h2>
	<!-- /wp:heading -->

	<!-- wp:paragraph {"align":"center"} -->
	<p class="has-text-align-center">Stay up to date with the latest from Ember & Oak Steakhouse.</p>
	<!-- /wp:paragraph -->

	<!-- wp:separator -->
	<hr class="wp-block-separator" />
	<!-- /wp:separator -->

	<!-- wp:social-links {"iconColor":"foreground","iconColorValue":"#374AC8","iconBackgroundColor":"primary","iconBackgroundColorValue":"#FFC3CF","size":"has-normal-icon-size","className":"is-style-pill-shape"} -->
	<ul class="wp-block-social-links has-normal-icon-size has-icon-color has-icon-background-color items-justified-center is-style-pill-shape">
		<!-- wp:social-link {"url":"https://facebook.com","service":"facebook"} /-->
		<!-- wp:social-link {"url":"https://twitter.com","service":"twitter"} /-->
		<!-- wp:social-link {"url":"https://instagram.com","service":"instagram"} /-->
	</ul>
	<!-- /wp:social-links -->
</div>
<!-- /wp:group -->