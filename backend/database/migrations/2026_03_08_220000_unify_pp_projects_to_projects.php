 ...
 *
 * NOTE: Each SQL statement is a separate DB::statement() call because Supabase
 * uses PgBouncer in transaction pooling mode, which does not allow multiple
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        //    Skip this step if pp_projects table does not exist (fresh install)
        // ---------------------------------------------------------------------------

        $ppProjectsExists = DB::select("
            SELECT EXISTS (
                SELECT FROM information_schema.tables
                WHERE table_schema = 'public'
                AND table_name = 'pp_projects'
            ) AS exists
        ");

        if ($ppProjectsExists[0]->exists ?? false) {
            DB::statement('
                WITH pp_norm AS (
                    SELECT
